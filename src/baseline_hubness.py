...
import random
import numpy as np
from rarity_score_update import *
...
score, score_index, max_r = manifold.rarity(k=nearest_k)
print(score[score_index])
print(max_r[score_index])
np.savetxt('./result/baseline.txt', score)
np.savetxt('./result/baseline_max_r.txt', max_r)
pd.Series(max_r).value_counts()
